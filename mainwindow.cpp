                                 ...
                                 QString *errOut)
{
    ...
    for (int i=0;i<totalFrames;++i){
        ...
        QTransform tr;                     // ← brand-new identity (no residual tilt)
        tr.translate(center.x(), center.y());
        tr.rotate(deg);                    // ← ONLY rotation this mode applies
        tr.translate(-center.x(), -center.y());
        p.setTransform(tr);
    ...
    QTemporaryDir tmp("gif_spin_XXXXXX"); if (!tmp.isValid()){ if (errOut) *errOut="Could not create temp directory."; return false; }
                                 ...
                                 QString *errOut)
{
    ...
    QImage back;
    const bool haveBack = !backImagePath.isEmpty()
                          && QFileInfo::exists(backImagePath)
                          && back.load(backImagePath);
    ...
    // Non-animated “flip”: just show back if provided
    if (!animate) {
        const QImage &face = haveBack ? backBase : frontBase;
        ...
        QTransform tr;
        tr.translate(center.x(), center.y());
        tr.scale(1.0, 1.0); // NO SHEAR → no tilt
        tr.translate(-center.x(), -center.y());
        p.setTransform(tr);
    ...
    // Animated flip: thickness floor so it never vanishes
    const int totalFrames = fps * durationSec;
    if (totalFrames < 1) { if (errOut) *errOut = "Total frames computed < 1."; return false; }
...
bool MainWindow::generateCompositeGif(const QString &frontImagePath,
                                      ...
                                      qreal zDegPerSec,
                                      bool useYaw,
                                      qreal maxYawDeg,      // interpreted as NUMBER OF YAW ROTATIONS
                                      bool useFlip,
                                      bool flipAnimate,
                                      int flipCycles,
                                      const QColor &bg,
                                      QString *errOut)
{
    ...
    const qreal eps = 0.08; // thickness floor
    const qreal yawRotations = useYaw ? qMax<qreal>(0.0, maxYawDeg) : 0.0;

    for (int i = 0; i < totalFrames; ++i) {
        const qreal t01 = (qreal)i / (qreal)totalFrames;

        // Z spin (deg/sec)
        const qreal zDeg = useZSpin ? (zDegPerSec * (qreal)i / (qreal)fps) : 0.0;

        // Yaw: X “thickness” + back/front
        qreal sxAbs = 1.0; bool yawBack = false;
        if (useYaw) {
            const qreal phiYawDeg = yawRotations * 360.0 * t01;
            const qreal cx        = qCos(qDegreesToRadians(phiYawDeg));
            yawBack = (cx < 0.0);
            sxAbs   = (1.0 - eps) * std::abs(cx) + eps;
        }

        // Flip: Y “thickness” + back/front
        qreal syAbs = 1.0; bool flipBack = false;
        if (useFlip) {
            if (!flipAnimate) {
                flipBack = true; // static back
                syAbs = 1.0;
            } else {
                const qreal phi = 2.0 * M_PI * qMax(1, flipCycles) * t01;
                const qreal cy  = qCos(phi);
                flipBack = (cy < 0.0);
                syAbs    = (1.0 - eps) * std::abs(cy) + eps;
            }
        }

        // Visible face: odd count of “through-edge” axes → back
        const bool showBack = (yawBack ^ flipBack);
        const QImage &face  = showBack ? backBase : frontBase;

        // Render (NO SHEAR anywhere → no tilt)
        QImage frame(frontBase.size(), QImage::Format_ARGB32_Premultiplied);
        frame.fill(bg);
...
void MainWindow::on_btnGenerate_clicked()
{
    ...
    const QString backPath = ui->editBackPath ? ui->editBackPath->text().trimmed() : QString();
   ...
   // QMessageBox::information(this, tr("Done"), tr("Created GIF:\n%1").arg(out));
